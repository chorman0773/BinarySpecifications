...
Version 1.1 makes no major changes to the specification from 1.0. The sole change in 1.1 is the addition of the Long Array Tag. 
...
All Binary Datatypes are written and read according to the LCLib Binary Data IO Specification, in Big Endian Byte Order Mode. The LCLib Binary Data IO Specification can be found [here](https://github.com/LightningCreations/lclib/blob/specification/Specification/BinaryDataIO.md). 
...
The current version of the specification is 1.1, and therefore the version for a file created in this version is [00 01]. 
...
Every ShadeNBT File shall contain a `Compound` Payload following the header. This Compound must contain a single nameless NBT Tag which is a `tag_compound`. This Compound is referred to as the Top Level Compound.
